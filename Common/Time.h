...
{
	return random(_t) / double(std::numeric_limits<uint32_t>::max()) * (_max - _min) + _min;
}

/// Pseudo-random factor in interval [0, _delta - 1], determined entire by _t.
inline int random(Time _t, int _delta)
{
	return _delta ? _delta > 0 ? random(_t) / (std::numeric_limits<uint32_t>::max() / _delta) : -(random(_t) / (std::numeric_limits<uint32_t>::max() / -_delta)) : 0;
}
...
//				d = t + Clock::duration() - m_start;
		return CHRONO::duration_cast<CHRONO::duration<int64_t, RATIO<1, s_baseRate>>>(d).count();
	}
private:
