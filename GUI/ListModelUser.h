...
public:
	...
	void setCurrentId(ModelId _id) { if (m_currentId != _id) { cdebug << "Switching current" << m_currentId << "becomes" << _id; m_currentId = _id; currentChanged(m_currentId); } }
	...
	void noteModelChanged() { indexDirty(); modelChanged(); if (m_currentId != UnknownModelId && index(m_currentId) == -1) setCurrentId(UnknownModelId); }
