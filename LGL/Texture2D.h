...
public:
	Texture2D() {}
	Texture2D(uSize const& _dims, foreign_vector<uint8_t> const& _data, GLenum _format = GL_RGBA, int _internalFormat = GL_RGBA, int _level = 0): Pimpl<TextureFace2D>(new TextureFace2D) { m_p->bindData(_dims, _data, _format, _internalFormat, _level); }
