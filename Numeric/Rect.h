...
public:
	...
	Rect shrunk(Numeric _f) const { return Rect(m_pos + fSize(_f, _f), m_size - 2 * _f); }
	Rect inset(Numeric _x, Numeric _y) const { fSize s(_x, _y); return Rect(m_pos + s, m_size - 2 * s); }
